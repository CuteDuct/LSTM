...
EPOCHS = 100
LEARNING_RATE = 0.001
# 获取当前脚本所在的目录 (也就是 LSTM 文件夹)
current_dir = os.path.dirname(os.path.abspath(__file__))
# 拼接出 data 文件夹的完整路径
DATA_PATH = os.path.join(current_dir, "data", "forecasting_sample", "data")

CLUSTER_THRESHOLD = 5.0      # 提取主轨迹的聚类阈值（米）
SMOOTH_WINDOW = 3            # 轨迹平滑窗口大小
...
# ===================== 2. 关键修复：提取主轨迹（解决震荡） =====================
def extract_main_trajectory(trajectory, CLUSTER_THRESHOLD=5.0):
    """通过层次聚类提取连续的主轨迹，过滤杂点"""
    if len(trajectory) < 5:
        return trajectory
    # 聚类：按坐标距离分组，层次聚类，计算欧几里得距离
    Z = linkage(trajectory, method='single', metric='euclidean') #single方法：最近距离法
...
#轻度平滑，只能消除小抖动
def smooth_trajectory(trajectory, SMOOTH_WINDOW=3):
...
class ArgoverseTrajectoryDataset(Dataset):
    ...
    def load_trajectories(self, data_path):
        ...
        #装处理好的轨迹、起点坐标、车道特征、运动学特征。
        print(f"🔍 正在加载数据集：{data_path}")
        files = self._get_csv_files(data_path)
        ...
        for file in files:
            ...
            traj = self._load_single_trajectory(full_path)
            if traj is None:
                continue

            # _preprocess_trajectory 现在返回两个值
            traj_norm, start_point = self._preprocess_trajectory(traj)
    ...
    def _get_csv_files(self, data_path):
        """获取所有CSV文件"""
        return [f for f in os.listdir(data_path) if f.endswith(".csv")]

    def _load_single_trajectory(self, file_path):
        """加载单个轨迹文件"""
        df = pd.read_csv(file_path)
        if "X" not in df.columns or "Y" not in df.columns:
            return None
        return df[["X", "Y"]].dropna().values

    def _preprocess_trajectory(self, traj):
        """预处理：主轨迹提取→平滑→归一化"""
        main_traj = extract_main_trajectory(traj, CLUSTER_THRESHOLD)
        smoothed = smooth_trajectory(main_traj, SMOOTH_WINDOW)
        normalized, start_points = normalize_trajectory(smoothed)
        return normalized, start_points
